...
export type CompetitionForAdmin = NonNullable<
  Awaited<ReturnType<typeof loadCompetitionWithLineups>>
>;

export type RecomputeCompetitionResult = {
...
export function extractEntries(raw: unknown): string[] {
...
export async function loadCompetitionWithLineups(competitionId: number) {
...
export async function loadCompetitionBasics(competitionId: number) {
  return prisma.competition.findUnique({
    where: { id: competitionId },
    include: {
      league: { select: { ownerId: true } },
...
export async function recomputeF1(competition: CompetitionForAdmin) {
...
export async function recomputeH2H(competition: CompetitionForAdmin) {
...
export async function recomputeCompetition(
  competitionId: number,
  mode: Mode,
): Promise<RecomputeCompetitionResult | null> {
  ...
  const result: RecomputeCompetitionResult = { competitionId, mode };

  if (runF1) {
    result.f1 = await recomputeF1(competition);
  }
  if (runH2H) {
    result.h2h = await recomputeH2H(competition);
  }
